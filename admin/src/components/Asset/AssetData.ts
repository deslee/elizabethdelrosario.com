...
export const getAssetType: (fileName: string | null | undefined) => AssetType = fileName => {
...
export function assetFilter(type: AssetType) {
    return (t: AssetWithData) => mime.lookup(t.uri || '').toString().toUpperCase().indexOf(type) === 0
}